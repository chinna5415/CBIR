...
class CBIR(tk.Frame):
    def __init__(self, root,pix):
        ...
        # main frame
        self.mainframe = tk.Frame(root,bg=self.bgc)
        self.mainframe.pack()

        # section frames
        self.topFrame = tk.Frame(self.mainframe,bg=self.bgc)
        self.topFrame.pack(side = 'right')
        self.bottomFrame = tk.Frame(self.mainframe,bg=self.bgc)
        self.bottomFrame.pack(side = 'left')

        # selected image
        self.selectedView = tk.Label(self.topFrame,width=450,
                                       height=pix.get_y(),bg=self.bgc)
        self.selectedView.pack(side='top')
        self.update_preview(self.imageList[0].filename)
        
        # control panel
        self.controlPanel = tk.Frame(self.topFrame,bg=self.bgc)
        self.controlPanel.pack()
        self.b_cc = tk.Button(self.controlPanel,text="Upload",
                           width=40,pady=self.bth,border=0,bg=self.btc,
                           fg=self.fgc,activebackground=self.abtc,
                           command=lambda:self.find_distance("CC"))
        self.b_cc.pack()
        self.l1 = tk.Label(self.controlPanel,bg=self.bgc,text=' ')
        self.l1.pack()
        self.b_inten = tk.Button(self.controlPanel,text="search",
                           width=40,border=0,pady=self.bth,bg=self.btc,
                           fg=self.fgc,activebackground=self.abtc,
                           command=lambda:self.find_distance("inten"))
        self.b_inten.pack()
        self.l2 = tk.Label(self.controlPanel,bg=self.bgc,text=' ')
        self.l2.pack()


        # results frame
        self.resultFrame = tk.Frame(self.bottomFrame,bg=self.bgc)
        self.resultFrame.pack()
        instr = tk.Label(self.resultFrame,bg=self.bgc,fg='#aaaaaa',
                text="Click image to select. Checkboxes indicate relevance.")
        instr.pack()
        self.canvas = tk.Canvas(self.resultFrame,bg=self.bgc,
                highlightthickness=0)

        # page navigation
        self.pageButtons = tk.Frame(self.bottomFrame,bg=self.bgc)
        self.pageButtons.pack()
        self.b_prev = tk.Button(self.pageButtons,text="<< Previous page",
                            width=30,border=0,bg=self.btc,
                            fg=self.fgc,activebackground=self.abtc,
                            command=lambda:self.prevPage())
        self.b_prev.pack(side='left')
        self.pageLabel = tk.Label(self.pageButtons,
                            text="Page 1 of " + str(self.totalPages),
                            width=43,bg=self.bgc,fg='#aaaaaa')
        self.pageLabel.pack(side='left')
        self.b_next = tk.Button(self.pageButtons,text="Next page >>",
                            width=30,border=0,bg=self.btc,
                            fg=self.fgc,activebackground=self.abtc,
    ...
    def reset(self):
        ...
        for img in self.imageList:
            self.checkList[img.filename] = tk.IntVar()
    ...
    def normalize_weight(self):
        ...
        for k,v in list(self.checkList.items()):
            if v.get() == 1:
                print((k + " selected"))
                pos = self.get_pos(k)
    ...
    def update_results(self,st):
        ...
        while photoRemain:
            ...
            for (filename, img) in photoRow:
                frame = tk.Frame(self.canvas,bg=self.bgc,border=0)
                frame.pack()
                link = tk.Button(frame,image=img,border=0,
                    bg=self.bgc,width=self.pix.get_xmax(),
                    activebackground=self.bgc)
                handler = lambda f=filename: self.update_preview(f)
                link.config(command=handler)
                link.pack(side='left')
                chkBtn = tk.Checkbutton(frame,variable=self.checkList[filename],
...
class PixInfo:
    def __init__(self, master):
        ...
        for infile in glob.glob('H:/Python/CBIR/temp/*.jpg'): # ./images/*.jpg
